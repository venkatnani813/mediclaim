   ...
   agent any
	...
	//}
		stage('Deploye-production'){
			steps{
	sshagent(['tomcat']) {
                       sh 'scp -o StrictHostKeyChecking=no /var/lib/jenkins/workspace/mediclain2/target/mediclaim-0.0.13-SNAPSHOT.jar ec2-user@172.31.35.90:/opt/apache-tomcat-9.0.40/webapps/'
			}
		}
		}
	}
